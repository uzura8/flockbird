{{#if next_id}}
<a href="#" data-uri="{{get_uri}}" data-get_data="{{conv2objStr 'max_id' next_id 'since_id' since_id 'image_size' image_size.key}}" data-list="#comment_list_{{parent.id}}" data-template="#comment-template" id="listMoreBox_comment_{{parent.id}}" class="listMoreBox js-ajax-loadList"><?php echo t('site.see_more'); ?></a>
{{/if}}
{{#each list}}
...
<?php endif; ?>
	...
	<a href="#" data-msg="<?php echo __('message_delete_confirm'); ?>" data-counter="#comment_count_{{../parent.id}}" data-uri="{{../delete_uri}}" data-parent="#commentBox_{{id}}" data-id="{{id}}" id="btn_comment_delete_{{id}}" class="btn btn-default boxBtn btn-xs js-ajax-delete hidden_btn"><i class="glyphicon glyphicon-trash"></i></a>
</div>
{{/each}}
