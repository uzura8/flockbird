...
class Form_SiteConfig
{
	public static function get_validation4edit_birthday($is_set_saved_value = false)
	{
		...
		$val->add($name, '生年月日設定を有効にするか', array('type' => 'radio', 'options' => $options_enable, 'value' => $value))
	...
	private static function get_site_config_values($name, $default_value = null)
	{
		...
		if (is_null($saved_value)) return $default_value;

		return $saved_value;
	}

	public static function get_site_configs_birthday_names()
	{
		return \Form_Util::get_field_names(self::get_validation4edit_birthday());
	}

	public static function save_site_configs_birthday(\Validation $val, $posted_values)
	{
